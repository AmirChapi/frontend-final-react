...
export default function CourseForm() {
  ...
  const handleSubmit = (e) => {
    ...
    const storedCourses = JSON.parse(localStorage.getItem("coursesList")) || [];
    const isDuplicate = !courseToEdit && storedCourses.some((c) => c.courseCode === formData.courseCode);
    ...
    const updatedCourses = courseToEdit
      ? storedCourses.map((c) => (c.courseCode === courseToEdit.courseCode ? formData : c))
      : [...storedCourses, { ...formData, assignedStudents: [] }];
      ...
      <Paper elevation={3} sx={{ padding: 4, width: 400, borderRadius: 2 }}>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <TextField
            ...
            fullWidth
            error={errors.year}
            helperText={errors.year ? "Year must be 4 digits and valid" : ""}
          />
